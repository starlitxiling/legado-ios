...
<task>
目标：只读复审阶段 4 单元 B13「备份上传与设置对齐」的未提交改动（git diff HEAD 与未跟踪文件中属于 B13 的部分），对照 Kotlin（commit cb664b84d，只读，前缀 /Users/wujie/Work/legado-ios/app/src/main/java/io/legado/app/）找出与 Android 行为 / 文件格式不一致、逻辑错误、并发缺陷、测试固化错误期望的问题；并**盘点设置项缺口**。工作目录 /Users/wujie/Work/legado-ios/.claude/worktrees/ios。
