...
背景与入口（Kotlin 为规格，只读，commit cb664b84d）：/Users/wujie/Work/legado-ios/app/src/main/java/io/legado/app/help/storage/{Backup,Restore,BackupConfig}.kt（备份包内容：bookshelf.json、bookSource.json、rssSource.json、replaceRule.json、txtTocRule.json、readRecord.json、bookmark.json、httpTTS.json、dictRule.json、config.xml 等；zip 文件名 backup 与 legado 前缀规则、恢复顺序、字段兼容）、lib/webdav/{WebDav,WebDavFile,Authorization}.kt（PROPFIND 深度 1 解析 multistatus XML、GET 下载、PUT 上传、MKCOL、exists、Basic 认证、URL 编码、坚果云等服务器的兼容注意）、help/AppWebDav.kt（默认目录 legado、备份文件命名 backup{date}.zip 与 legado 前缀、按 serverID 选凭据）。
