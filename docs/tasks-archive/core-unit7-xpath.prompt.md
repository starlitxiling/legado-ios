...
背景与入口：
- 规格 docs/spec/rule-engine.md §4 XPath 模式、§6（AnalyzeRule 层已做递归合并）、§11「未确定 U7：JsoupXpath 扩展函数集与 asString() 格式」。Kotlin 只读：/Users/wujie/Work/legado-ios/app/src/main/java/io/legado/app/model/analyzeRule/AnalyzeByXPath.kt（155 行：文档来源既可能是 jsoup Element 也可能是字符串重新解析；结果 asString / 元素列表的转换），commit cb664b84d。JsoupXpath 在 jsoup DOM 上执行 XPath 1.0 并扩展了若干函数（如 text() 的特殊语义、num()、allText()、outerHtml()、html() 等），请从 Kotlin 调用与你对该库的把握列出需要支持的扩展，标注哪些未核实。
