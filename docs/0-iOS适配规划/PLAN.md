...
> 状态：**方向已拍板**（2026-09-12，见 §7），§6 剩余三项待补，不阻塞阶段 0 启动。
...
- 补齐 §6 剩余三项，写入 §7；在远端新建 orphan 分支 `ios`（对外动作，须人类同意后执行）。
- 建立 **一致性测试语料**：从 `app/src/test/java/io/legado/app/model/analyzeRule/` 的 6 个测试抽黄金用例；再挑 20 到 30 个覆盖不同特征的公开书源（纯 CSS、XPath、JSONPath、重 JS、登录、webView），**用录制的 HTTP fixture 离线跑**，不发真实网络请求。每个用例记录 Android 端的输出作为期望值。
...
1. **MVP 范围**：是否接受 §4 阶段 3 的边界（网络文字书源 + 书架 + 阅读 + 替换规则 + 备份恢复），以及阶段 4 各项的优先级。
2. **团队与工期**：参与人数、Swift 熟练度、期望里程碑，用于给各阶段填工期。
3. **最低系统版本与设备**：iOS 最低版本、是否支持 iPad。
...
| 仓库形态 | **本仓库新开 `ios` 分支** | 按 **orphan 分支**理解（Swift 工程独占一棵树，不含 Android 代码），一致性语料引用 Kotlin 端时记录 `master` 的 commit hash。若人类本意是基于 `master` 的普通分支加 `ios/` 目录，开阶段 0 前改一句即可 |
