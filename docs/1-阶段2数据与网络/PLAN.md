...
> 状态：**待人类确认**。确认前不写代码。轮次在 `ios` 分支直接进行（等价 `/start --no-worktree`），因为 `ios` 是 orphan 分支，按主分支开 worktree 会切到 Android 代码。
...
1. **WebDAV 冒烟环境**：是否有可用的 WebDAV 测试账号（坚果云 / 自建）？没有则 U6 只做假客户端测试，真连留给人类。
2. **人工冒烟书源**：由人类挑一个公开书源 JSON 在真机跑「搜索 → 正文」；我不挑站点。
3. **单元顺序是否接受**（U1 → U6），以及是否允许 U3 与 U1 并行（文件不重叠，可并行）。
