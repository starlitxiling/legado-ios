...
- **技术栈**：Swift 5.9+ / SwiftUI；规则引擎 JS 宿主用 JavaScriptCore；HTML 解析 SwiftSoup（对齐 jsoup 1.23.2 语义）、XPath 用 Kanna（libxml2）；数据库 GRDB；网络 URLSession。**最低 iOS 17，Universal**（iPhone 优先布局，iPad 只保证可用）。分发目标为自签侧载，不以 App Store 过审为设计约束。
- **目录规划**（规划中，尚未建立）：
