...
/// 规格 §4；Kotlin AnalyzeRule.kt:367-372：仅反转义带分号的 HTML4 实体。
enum HTML4Entities {
    static func unescape(_ text: String) throws -> String {
        guard text.contains("&") else { return text }
        let pattern = try NSRegularExpression(pattern: #"&(#(?:[xX][0-9a-fA-F]+|[0-9]+)|[A-Za-z][A-Za-z0-9]*);"#)
        ...
        for match in matches {
            ...
            if token.hasPrefix("#x") || token.hasPrefix("#X") {
                code = UInt32(token.dropFirst(2), radix: 16)
            } else if token.hasPrefix("#") {
                code = UInt32(token.dropFirst())
            } else {
                code = named[token]
            }
    ...
    // HTML4 的 252 个名称与码点；依据 CPython 3.9 html.entities.name2codepoint 的标准映射。
    // 不包含 HTML5 的 apos，也不执行无分号实体或递归反转义。
