import Foundation
import GRDB

public struct RssStar: Codable, Equatable, Sendable, FetchableRecord, PersistableRecord {
    public static let databaseTableName = "rssStars"
    public var origin: String = ""
    public var sort: String = ""
    public var title: String = ""
    public var starTime: Int64 = 0
    public var link: String = ""
    public var pubDate: String? = nil
    public var description: String? = nil
    public var content: String? = nil
    public var image: String? = nil
    public var group: String = "默认分组"
    public var variable: String? = nil
    public var type: Int = 0
    public var durPos: Int = 0
    public init() {}
    private enum CodingKeys: String, CodingKey {
        case origin, sort, title, starTime, link, pubDate, description, content, image, group, variable, type, durPos
    }
    public init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        origin = try c.gsonString(forKey: .origin) ?? ""
        sort = try c.gsonString(forKey: .sort) ?? ""
        title = try c.gsonString(forKey: .title) ?? ""
        starTime = try c.gsonLong(forKey: .starTime) ?? 0
        link = try c.gsonString(forKey: .link) ?? ""
        pubDate = try c.gsonString(forKey: .pubDate)
        description = try c.gsonString(forKey: .description)
        content = try c.gsonString(forKey: .content)
        image = try c.gsonString(forKey: .image)
        group = try c.gsonString(forKey: .group) ?? "默认分组"
        variable = try c.gsonString(forKey: .variable)
        type = try c.gsonInt(forKey: .type) ?? 0
        durPos = try c.gsonInt(forKey: .durPos) ?? 0
    }
    public init(row: Row) {
        origin = row["origin"]
        sort = row["sort"]
        title = row["title"]
        starTime = row["starTime"]
        link = row["link"]
        pubDate = row["pubDate"]
        description = row["description"]
        content = row["content"]
        image = row["image"]
        group = row["group"]
        variable = row["variable"]
        type = row["type"]
        durPos = row["durPos"]
    }
}

public extension RssStar {
    var article: RssArticle {
        var value = RssArticle(origin: origin, link: link, title: title)
        value.sort = sort; value.pubDate = pubDate; value.description = description
        value.content = content; value.image = image; value.group = group
        value.variable = variable; value.type = type; value.durPos = durPos
        return value
    }
}
